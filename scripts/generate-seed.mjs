/**
 * Generates drizzle/seed.sql from a single in-script seed dataset.
 * Run via `pnpm gen:seed` (alias for `node scripts/generate-seed.mjs`).
 *
 * Keep this aligned with `src/lib/seed-data.ts` — the duplication is intentional:
 * the runtime/admin path uses TypeScript imports; the SQL emitter is a flat
 * JS script so it can run with no ts-loader.
 */
...
const SEED_POSTS = [
  ...
  },
  {
    ...
    body: SHORT_BODY("我的 2026 AI Coding 工作流", "一年下来的工具沉淀清单。"),
  },
  {
    ...
    body: SHORT_BODY("PDB 文件格式逆向笔记 (一)", "MSF / Stream Directory / TPI&IPI 流。"),
  },
  {
    ...
    body: SHORT_BODY("关于 Agentic IDE，我想得有点多", "范式迁移的随想。"),
  },
  {
    ...
    body: SHORT_BODY("用 Modern C++ 包装 Win32：一份风格指南", "RAII / expected / awaitable。"),
  },
  {
    ...
    body: SHORT_BODY("Context Engineering", "上下文窗口的工程化。"),
  },
  {
    slug: "wsl2-dev-setup",
    ...
    body: SHORT_BODY("WSL2 + Windows Terminal 开发环境", "dotfiles 与桥接技巧。"),
  },
  {
    ...
    body: SHORT_BODY("以十年为单位思考", "把时间尺度拉长。"),
  },
  {
    ...
    body: SHORT_BODY("重新审视 DirectX 中的 COM", "二十年前的范式为什么仍然成立。"),
  },
  {
    ...
    body: SHORT_BODY("论读源码", "从你疼痛的那一行开始。"),
  },
  {
    ...
    body: SHORT_BODY("Claude Code Power-User Tips", "Hooks / 子代理 / slash。"),
  },
];
...
const tsToUnix = (iso) => Math.floor(new Date(iso).getTime() / 1000);
...
for (const p of SEED_POSTS) {
  const { words, readTime } = analyze(p.body);
  const ts = tsToUnix(p.publishedAt);
  out += `INSERT INTO posts (slug, title, excerpt, body, tag_id, series, status, pinned, words, read_time, published_at, created_at, updated_at) VALUES (${sql(p.slug)}, ${sql(p.title)}, ${sql(p.excerpt)}, ${sql(p.body)}, ${sql(p.tagId)}, ${p.series ? sql(p.series) : "NULL"}, 'published', ${p.pinned ? 1 : 0}, ${words}, ${readTime}, ${ts}, ${ts}, ${ts});\n`;
}
out += "\n";

for (const proj of SEED_PROJECTS) {
  out += `INSERT INTO projects (name, description, language, stars, year, state, url, sort) VALUES (${sql(proj.name)}, ${sql(proj.description)}, ${sql(proj.language)}, ${sql(proj.stars)}, ${sql(proj.year)}, ${sql(proj.state)}, ${proj.url ? sql(proj.url) : "NULL"}, ${proj.sort});\n`;
}

const outPath = join(repoRoot, "drizzle", "seed.sql");
mkdirSync(dirname(outPath), { recursive: true });
writeFileSync(outPath, out, "utf8");
console.log(`Wrote ${outPath} (${out.length} bytes, ${SEED_POSTS.length} posts, ${SEED_PROJECTS.length} projects).`);
